...
DWORD CCell::Select(int Which)
{
	...
	for (i = 0; i < c; i++)
		...
		if (Which == 98)
			...
			if (t == LAYER_PAINT)
				break;
			if ((t >= LAYER_MATTE0) && (t <= LAYER_MATTE9))
				break;
			}
#ifdef FBVER7
		else if (Which == 97)
			{
			if (t == LAYER_OLD_OVERLAY)
				break;
			if (t == LAYER_OLD_BG)
				break;
			}
#endif
		else if (t == (DWORD)Which)
			break;
		}
	if (i < c)
		k = p->layers[i].dwKey;  // found something
	else if (c && k)			// no thumb, but something 
		k = 0;
	return k;
}
