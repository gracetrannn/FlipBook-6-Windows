...
void CLayers::GetLayerImage(BYTE* pBits, UINT kind) {
    ...
    for(int y = m_maxy; y >= m_miny; --y) {
        ...
        for(int x = m_minx; x < m_maxx; ++x) {
            BYTE color[4];
            pPals->Color(color, pImage[(m_maxy - y)*m_width + x], x, y);
            color[3] = pAlpha[(m_maxy - y)*m_width + x];
            if(color[3] > 0) { // copy only non-transparent pixels
