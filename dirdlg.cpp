// DirDlg.cpp - implements class CDirDialog
...
#include "DirDlg.h"      // Our own header file
...
void CDirDialog::OnInitDone()
{
	...
	CWnd * pTemp;
	if ((pTemp = pp->GetDlgItem(nId = edt1)) == NULL)
		pTemp = pp->GetDlgItem(nId = cmb13);	// new er vc and .net fix
    // Create a new edit control where edt1 now is
    ASSERT(pTemp);
    pTemp->GetWindowRect(rct); //Get edt1 rectangle
...
//  m_Edit.SetSel(0, strlen(m_ofn.lpstrInitialDir));
    ...
    m_Open.Create(_T("Save"), WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
                    rct, pp, IDC_OPEN);
...
//#define DIRDIALOG_TESTING
    ...
    pTemp->GetWindowRect(rct);
    pp->ScreenToClient(rct);
    pTemp->SetWindowPos(NULL, rct.left, rct.top+70,
                   0, 0, SWP_NOZORDER | SWP_NOSIZE);
...
void CDirDialog::OnFolderChange()
{
    ...
    if (len > 0 && m_strPath[len-1] != NATIVE_SEP_CHAR)
    {
        m_strPath += NATIVE_SEP_STRING;
        ++len;
    }
#ifndef FLIPBOOK_MAC
